...
binary search tree: for each node, all elements in its left subtree are less than node, all on right are greater than node
*/
class Node {
  public:
  int key;
...
Node* insertNode(Node* root, int key){
  if(root==NULL){root = createNode(key);return root;}  //case for node not being created properly
  queue<Node*> q;
  q.push(root); //adding node to queue
  while(!q.empty()){
    Node* temp = q.front();q.pop();
    if(temp->left != NULL)q.push(temp->left); //if node exists, add it to quene if not create it
    else{temp->left=createNode(key);return root;}
    if(temp->right !=NULL)q.push(temp->right);
    else{temp->right=createNode(key);return root;}
  }return NULL;
}
...
void sortTree(Node* root){
  ...
  i=0;arrayToTree(root, temp, &i);
  delete[] temp;
}
bool lookUp(Node* root, int num){
  if(root==NULL)return false;
  if(num==root->key)return true;
