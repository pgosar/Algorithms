...
# uses binary heap to lower time complexity from O(v^2) to the O((e+v)*logv)
...
class Heap:
    ...
    def decreaseKey(
        self, v, dist
    ):  # decreases key of a node, necessary for edge relaxation
    ...
    def dijkstra(self, source):
        ...
        while newMinHeap.isEmpty() == False:
            ...
            for i in self.graph[x]:
                ...
                y = i[0]
                if (
                    newMinHeap.checkMinHeap(y) == True
                    and distance[x] != sys.maxsize
                    and i[1] + distance[x] < distance[y]
                ):
